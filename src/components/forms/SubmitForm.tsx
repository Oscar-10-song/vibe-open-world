  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      <fieldset className="mb-10">
        <legend className="text-lg font-semibold text-[var(--color-text)] mb-5">
          📦 Project Info
        </legend>
        <div className="space-y-4">
          ...
          {screenshotPreview && !errors.screenshot_url && (
            <div className="relative aspect-[16/9] rounded-lg overflow-hidden bg-[var(--color-bg-tertiary)] border border-[var(--color-border)] max-w-md">
              <img
              ...
              {POPULAR_TECH_STACKS.filter(t => !techStack.includes(t))
                ...
                .map(tech => (
                  <button
                    ...
                    className="text-xs px-2 py-1 rounded-md bg-[var(--color-bg-tertiary)] text-[var(--color-text-tertiary)] hover:text-[var(--color-accent)] hover:bg-[var(--color-accent)]/10 transition-colors"
                  >
                    + {tech}
                  </button>
                ))}
      ...
      {/* ======== Error ======== */}
      {submitState === 'error' && (
        <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-sm text-red-600 dark:text-red-400 mb-6">
          {errorMessage}
        </div>
      )}
